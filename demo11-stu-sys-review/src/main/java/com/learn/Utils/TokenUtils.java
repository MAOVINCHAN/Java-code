...
public class TokenUtils {
    private static String secret; // 强密码
    private static long expiration = 60 * 60 * 1000; // 1小时

    static {
        try {
            InputStream is = TokenUtils.class.getResourceAsStream("/jwtSecret.properties");
            Properties pp = new Properties();
            pp.load(is);
            secret = (String) pp.get("secrete");
        } catch (IOException e) {
