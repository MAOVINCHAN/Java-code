	...
	$(function() {	
	    ...
	    $("#delete").click(function(){
	  	...
	  	$("#clazzList").combobox({
	  		...
	  		onChange: function(newValue, oldValue){
	  			//加载班级下的学生
	  			$('#dataList').datagrid("options").queryParams = {cid: newValue};
	  			$('#dataList').datagrid("reload");
	  		}
	  	});
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							let gid = $("#add_gradeList").combobox("getValue");
							let cid = $("#add_clazzList").combobox("getValue");
							$.ajax({
								type: "post",
	    ...
	    $("#editDialog").dialog({
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						let gid = $("#edit_gradeList").combobox("getValue");
						let cid = $("#edit_clazzList").combobox("getValue");
						if(!validate){
							$.messager.alert("消息提醒","请检查你输入的数据!","warning");
							return;
						} else{
			...
			onBeforeOpen: function(){
				...
				let gid = selectRow.grade.gid;
				let cid = selectRow.clazz.cid;
