	...
	<script type="text/javascript">
		$(function() {
			...
			$('#dataList').datagrid({
				...
				fit: true,//自动大小
				method: "get",
				url:"/teacher?action=data&t="+new Date().getTime(),
				idField:'tid',
				...
				columns: [[
					...
					{field:'courseList',title:'课程',width:500,
						formatter: function(value,row,index){
							if (row.courseList){
								let courseList = row.courseList;
								let course = "";
								for(let i = 0;i < courseList.length;i++){
									let gradeName = courseList[i].grade.gradeName;
									let clazzName = courseList[i].class.clazzName;
									let courseName = courseList[i].course.courseName;
						...
						handler:function(){
							...
							if(!validate){
								...
								$(table).find(".chooseTr").each(function(){
									let gradeid = $(this).find("input[textboxname='gradeid']").attr("gradeId");
									let clazzid = $(this).find("input[textboxname='clazzid']").attr("clazzId");
									let courseid = $(this).find("input[textboxname='courseid']").attr("courseId");
									let course = gradeid+"_"+clazzid+"_"+courseid;
									chooseCourse.push(course);
								});
								let number = $("#add_number").textbox("getText");
								let name = $("#add_name").textbox("getText");
								let gender = $("#add_sex").textbox("getText");
								let phone = $("#add_phone").textbox("getText");
								let qq = $("#add_qq").textbox("getText");
								let data = {number:number, name:name,gender:gender,phone:phone,qq:qq,course:chooseCourse};

								$.ajax({
									type: "post",
									url: "/teacher?action=add",
									data: data,
									success: function(data){
										if(data.status == 200){
				...
				buttons: [
					{
						...
						handler:function(){
							...
							//获取新选择的课程
							let gradeId = $("#add_gradeList").combobox("getValue");
							let clazzId = $("#add_clazzList").combobox("getValue");
							let courseId = $("#add_courseList").combobox("getValue");
							let newChoose = gradeId+"_"+clazzId+"_"+courseId;
							...
							//添加到表格显示
							let tr = $("<tr class='chooseTr'><td>课程:</td></tr>");

							let gradeName = $("#add_gradeList").combobox("getText");
							let gradeTd = $("<td></td>");
							let gradeInput = $("<input style='width: 200px; height: 30px;' data-options='readonly: true' class='easyui-textbox' name='gradeid' />").val(gradeName).attr("gradeId", gradeId);
							$(gradeInput).appendTo(gradeTd);
							$(gradeTd).appendTo(tr);

							let clazzName = $("#add_clazzList").combobox("getText");
							let clazzTd = $("<td></td>");
							let clazzInput = $("<input style='width: 200px; height: 30px;' data-options='readonly: true' class='easyui-textbox' name='clazzid' />").val(clazzName).attr("clazzId", clazzId);
							$(clazzInput).appendTo(clazzTd);
							$(clazzTd).appendTo(tr);

							let courseName = $("#add_courseList").combobox("getText");
							let courseTd = $("<td></td>");
							let courseInput = $("<input style='width: 200px; height: 30px;' data-options='readonly: true' class='easyui-textbox' name='courseid' />").val(courseName).attr("courseId", courseId);
							$(courseInput).appendTo(courseTd);
							$(courseTd).appendTo(tr);

							let removeTd = $("<td></td>");
							let removeA = $("<a href='javascript:;' class='easyui-linkbutton removeBtn'></a>").attr("data-options", "iconCls:'icon-remove'");
			...
			$("#editDialog").dialog({
					...
					},
					{
						...
						handler:function(){
							...
							if(!validate){
								...
								let tid = $("#dataList").datagrid("getSelected").tid;
								let number = $("#edit_number").textbox("getText");
								let name = $("#edit_name").textbox("getText");
								let gender = $("#edit_sex").textbox("getText");
								let phone = $("#edit_phone").textbox("getText");
								let qq = $("#edit_qq").textbox("getText");
								let data = {tid, number, name,gender,phone,qq,course:chooseCourse || []};
								console.log("data", data);

								$.ajax({
									type: "post",
									url: "/teacher?action=edit",
									data: data,
									success: function(data){
										if(data.status == 200){
				...
				onBeforeOpen: function(){
					...
					let courseList = selectRow.courseList;

					for(let i = 0;i < courseList.length;i++){
						let gradeId = courseList[i].grade.gid;
						let gradeName = courseList[i].grade.gradeName;
						let clazzId = courseList[i].class.cid;
						let clazzName = courseList[i].class.clazzName;
						let courseId = courseList[i].course.cid;
						let courseName = courseList[i].course.courseName;
						//添加到表格显示
						let tr = $("<tr class='chooseTr'><td>课程:</td></tr>");
